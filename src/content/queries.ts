        ...
        queries: [
            {
                ...
                description: "返回 4 行 (T/FDN, Level 1, Level 2, Roof), 按标高升序排列",
...
ORDER BY ?elev`
            },
            {
                ...
                description: "OPTIONAL 内嵌套 OPTIONAL 避免因 label 缺失导致整块 OPTIONAL 失效.",
...
ORDER BY ?storeyLabel ?spaceLabel`
            },
            {
                ...
                description: "直接用 STRAFTER 从 URI 提取名称",
...
ORDER BY DESC(?count)`
            },
            {
                ...
                description: "仅有类声明 (owl:Class). 以下仅查询实际存在的实例类型.",
...
ORDER BY DESC(?count)`
            },
            {
                ...
                description: "rdf:type 三元组, 导致同一构件被重复返回. 固定类型 UNION + DISTINCT 可避免此问题.",
...
WHERE {
    {
        ...
        WHERE {
            ...
            UNION
            {
                ...
                ficr:req_pg1b_floor ficr:hasREI ?r .
...
ORDER BY ?category ?status`
            },
            {
                ...
                description: "Doorset 无 REI 属性，actualREI/requiredREI 以 0/1 占位 (实意: 遮挡 vs 未遮挡)",
...
WHERE {
    ...
    UNION
    {
        ...
        ?space bot:adjacentElement ?elem ;
               rdfs:label ?spaceLabel .
        ficr:req_pg1b_floor ficr:hasREI ?reqREI .
        ...
        queries: [
            {
                ...
                description: "declaredExposure_GBP: RiskUnit 级声明风险敞口 (= 覆盖空间面积之和 × 标准单价)",
...
ORDER BY ?ruLabel ?spaceLabel`
            },
            {
                ...
                description: "无类型过滤时 SPARQL 会同时匹配两类节点导致计数虚高.",
...
ORDER BY ?ruLabel ?conditionState`
            },
            {
                ...
                description: "会混入结果. 修复后精确返回 4 条 BoundaryAssumption.",
...
ORDER BY ?ruLabel ?systemName`
            },
            {
                ...
                description: "预期 4 条 (cert_comp_A, drawing_struct_A, obs_cavity_B, drawing_struct_B).",
...
ORDER BY ?ruLabel ?assumptionType`
            },
            {
                ...
                description: "NOTE: Roof Space Unit (riskunit_roof) 无 BoundaryAssumption → 不出现在本查询.",
