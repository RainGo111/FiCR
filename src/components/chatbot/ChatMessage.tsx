...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  if (isUser) {
    const isJson = message.content && (message.content.trimStart().startsWith('{') || message.content.trimStart().startsWith('['));
    const isPlainText = message.content && !isJson;

    return (
      <div className="flex justify-end mb-4">
        <div className="flex items-start gap-3 max-w-[80%]">
          <div className="bg-primary-600 rounded-lg rounded-tr-sm px-5 py-3 shadow-sm">
          ...
          <div className="bg-primary-600 p-2 rounded-full shrink-0 shadow-sm">
  ...
  return (
    <div className="flex justify-start mb-4">
      <div className="flex items-start gap-3 max-w-[85%]">
        <div className="bg-slate-700 p-2 rounded-full shrink-0 shadow-sm">
          <Bot className="w-4 h-4 text-white" />
        </div>
        <div className="bg-white rounded-lg rounded-tl-sm px-5 py-4 shadow-sm border border-slate-200 min-w-[300px]">
          {/* Pipeline progress */}
          ...
          {!message.isStreaming && message.content && message.stages && (
            <div className="mt-4 pt-3 border-t border-slate-200">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-600 bg-slate-50 border border-slate-200 rounded-md hover:bg-slate-100 hover:border-slate-300 hover:text-slate-800 transition-smooth active:scale-95"
              >
