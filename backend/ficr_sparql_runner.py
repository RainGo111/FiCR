...
PROBES = {
    "A1": (
        "MultiStoreyBuilding exists",
        _PFX + "ASK { ?b a ficr:MultiStoreyBuilding }",
    ),
    "A2": (
    ...
    "A5": (
        "At least one element instance (Wall/FloorSlab/RoofSlab/Doorset/etc.)",
        _PFX + """ASK {
            { ?e a ficr:Wall } UNION { ?e a ficr:FloorSlab } UNION
            { ?e a ficr:RoofSlab } UNION { ?e a ficr:Doorset } UNION
            { ?e a ficr:Ceiling } UNION { ?e a ficr:Window }
        }""",
    ),
    "A6": (
        "Space with adjacentElement or equipment",
        _PFX + """ASK {
            { ?sp bot:adjacentElement ?el }
            UNION
            { ?e ficr:islocatedIn ?sp }
        }""",
    ),
    "B1": (
        "Wall or FloorSlab with hasREI AND regulatory requirement",
        _PFX + """ASK {
    ...
    "B2": (
        "Wall or FloorSlab with hasREI for element-level detail",
        _PFX + """ASK {
            { ?w a ficr:Wall ; ficr:hasREI ?v }
            { ?req a ficr:RegulatoryRequirement ; ficr:hasREI ?r }
        }""",
    ...
    "B3": (
        "OWL equivalentClass definitions exist in TBox",
        _PFX + "ASK { ?c owl:equivalentClass ?def }",
    ),
    "C1": (
    ...
    "C3": (
        "BoundaryAssumption with supportedByEvidence or condition",
        _PFX + """ASK {
    ...
    "D1": (
        "Event triggering a Task",
        _PFX + """ASK {
            ?ev a/rdfs:subClassOf* ficr:Event ;
                ficr:triggersTask ?t
        }""",
    ),
    "D2": (
        "ComplianceChecking with hasComplianceResult",
        _PFX + """ASK {
            ?a a ficr:ComplianceChecking ;
               ficr:hasComplianceResult ?r
        }""",
    ),
}
...
def _to_json(val):
    ...
    if isinstance(val, Literal):
        ...
        if hasattr(py, "as_integer_ratio") and not isinstance(py, (int, float)):
            return float(py)
        # date/datetime → ISO string for JSON compat
