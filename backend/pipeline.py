...
_ROOT = _HERE.parent
SYSTEM_PROMPT_PATH = _HERE / "prompts" / "1_survey_extractor.md"
SCHEMA_PATH = _HERE / "schemas" / "survey_schema.json"
TBOX_PATH = _ROOT / "ontology" / "ficr_tbox.ttl"
REG_PATH = _HERE / "references" / "ficr_regulatory_config.ttl"
SPARQL_PATH = _HERE / "references" / "ficr_risk_discovery_queries.sparql"
OUTPUT_DIR = _HERE / "sessions"

MAX_VALIDATION_RETRIES = 2


class GapReport(Exception):
    """Raised when LLM#1 returns a Mode B gap report instead of JSON."""

    def __init__(self, report_text: str):
        self.report_text = report_text
        super().__init__("LLM returned a gap report (Mode B)")


def _is_gap_report(text: str) -> bool:
    """Check if LLM output is a Mode B gap report (no JSON, has markdown)."""
    s = text.strip()
    has_json = "{" in s and "}" in s
    has_markdown = s.startswith("#") or "\n## " in s
    return not has_json and has_markdown and len(s) > 100

# ── LLM Report Prompt (LLM #3) — loaded from prompts/3_report_narrator.md ──
...
class LLMAdapter:
    ...
    def chat_multi(self, system: str, messages: list[dict]) -> str:
        """Send a system prompt with a full message history and return the response."""
        self._ensure_client()
...
def stage_llm1(llm: LLMAdapter, user_input: str,
    ...
    for attempt in range(1 + MAX_VALIDATION_RETRIES):
        ...
        try:
            ...
            # Mode B: LLM intentionally returned a gap report (first attempt only)
            if attempt == 0 and _is_gap_report(raw):
                raise GapReport(raw)
...
def stage_llm2(llm: LLMAdapter, sparql_results: dict) -> str:
    ...
    report = llm.chat(_REPORT_PROMPT, user_msg)
...
def run_pipeline(
    ...
    except GapReport as gr:
        print(f"  [Mode B] Gap report returned ({len(gr.report_text)} chars)")
        print(f"\n{'='*60}")
        print(f"  Pipeline complete (Mode B — gap report)")
        print(f"{'='*60}\n")
        return {
            "survey": None,
            "abox_path": None,
            "sparql_results": None,
            "report": gr.report_text,
            "mode": "B",
        }
    print()
    ...
    if generate_report:
        ...
        if report_mode == "deterministic":
            report = report_generator.generate_report(sparql_results)
            print(f"  [Report] Deterministic report generated ({len(report)} chars)")
        else:
...
""")
    ...
    ap.add_argument("--report-mode", default="deterministic",
                    choices=["deterministic", "llm"],
                    help="Report mode: deterministic (no LLM) or llm (default: deterministic)")
    ...
    if args.survey_json:
        ...
        if not args.no_report:
            print("── Stage 4: SPARQL → Report ──")
            if args.report_mode == "deterministic":
                report = report_generator.generate_report(sparql_results)
                print(f"  [Report] Deterministic report generated ({len(report)} chars)")
            else:
                default_models = {
