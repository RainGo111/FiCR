"""Quick test: load TBox + regulatory config + ABox, run all 13 CQ queries (A1-A6, B1-B3, C1-C4)."""
...
def load_merged_graph():
    g = Graph()
    g.parse(str(PROJECT_ROOT / "ontology/ficr_tbox.ttl"), format="turtle")
    g.parse(str(ROOT / "references/ficr_regulatory_config.ttl"), format="turtle")
    g.parse(str(ROOT / "sessions/duplex_a/abox.ttl"), format="turtle")
